...
class NaoLocomotionEnv(DirectRLEnv):
    cfg: DirectRLEnvCfg
    ...
    def __init__(self, cfg: DirectRLEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Initialize actions tensor to prevent shape mismatch on first observation
        self.actions = torch.zeros(
            (self.num_envs, len(self.cfg.joint_gears)),
            dtype=torch.float32,
...
def compute_rewards(
    ...
    total_reward = torch.where(
        reset_terminated, torch.ones_like(total_reward) * death_cost, total_reward
    )
    return total_reward
